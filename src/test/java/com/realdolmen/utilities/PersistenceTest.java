...
/**
 * Takes care of some streamlining for getting persistence tests to work
 * correctly in all cases. These measures would likely be taken by a real world
 * project as well, but they are not relevant to the essence of JPA.
 *
 * Advanced course participants are free to explore this code to get some ideas.
 * Beginner course participants can simply ignore the contents of this file, and
...
public abstract class PersistenceTest extends Assert {
	...
	/**
	 * Chooses the database engine to run this unit test. Default lets
	 * {@link com.realdolmen.course.utilities.persistence.PersistenceTest.DatabaseEngine#current()}
	 * choose. Subclasses may override this, if they know what they're doing :)
	 * 
	...
	/**
	 * Recreates the SQL schema from scratch. One might wonder why this is
	 * necessary when using a JPA property such as "hbm2ddl.auto". This is due
	 * to the fact that sometimes a drop-create fails to delete old references.
	 * For example, when a previous entity is renamed, the drop-create feature
	 * would not "see" that the table that corresponded with the old entity name
	 * still exists. This thus creates "lingering" tables which sporadically
	 * causes trouble due to foreign key violations. This step may not _always_
	 * be necessary, but it _is_ necessary _sometimes_. As this course is not
	 * about dealing with SQL foreign key side-effects, we take this measure to
	 * guarantee that problem never happens in the course.
	 */
	@Before
	...
	/**
	 * Obtains a <strong>new</strong> JDBC connection using connection settings
	 * defined in {@link #databaseEngine}. Note this connection does not
	 * participate in the same transaction as the
	 * {@link com.realdolmen.course.utilities.persistence.JpaPersistenceTest#entityManager()}
	 * , so be careful when asserting against both.
	 * 
	...
	/**
	 * Represents all available database engines that can be used for running
	 * unit tests. Switching implementations can be done using a
	 * {@link #DATABASE_ENGINE_SYSTEM_PARAMETER system parameter}. Remember:
	...
	public enum DatabaseEngine {
		...
		/**
		 * Fails when the expected database engine is not the currently selected
		 * one. Allows clients to mandate a specific environment, without them
		 * needing to violate their cohesion.
		 * 
		 * @param expected
		 *            The expected database engine.
		 * @param why
		 *            Please add a reason why you would have this almost
